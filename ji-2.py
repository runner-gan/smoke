...
class YoLov5TRT(object):
    ...
    def infer_result(self, image_raw):
        threading.Thread.__init__(self)
        # Make self the active context, pushing it on top of the context stack.
        self.ctx.push()
        # Restore
        ...
        batch_origin_w = []
        batch_input_image = np.empty(shape=[self.batch_size, 3, self.input_h, self.input_w])
        print("image_raw",type(image_raw))
        input_image, image_raw, origin_h, origin_w = self.preprocess_image(image_raw)
        batch_image_raw.append(image_raw)
        batch_origin_h.append(origin_h)
        batch_origin_w.append(origin_w)
...
def init():
    ...
    Returns: model
    """
    engine_file_path = "/project/train/models/best.engine"
   
    PLUGIN_LIBRARY = "/project/ev_sdk/model/tensorrtx/yolov5/build/libmyplugins.so"
...
def process_image(net=None, input_image=None, args=None, **kwargs):
    ...
    """
    result_boxes, result_scores, result_classid = net.infer_result(input_image)
    fake_result = {}
    ...
    for i in range(len(result_boxes)):
        xyxy, conf, cls = result_boxes[i], result_scores[i], int(result_classid[i])
        #label = names[cls]
...
if __name__ == '__main__':
    # Test API
    img =cv2.imread('/project/ev_sdk/model/tensorrtx/yolov5/samples/bus.jpg')
    predictor = init()
    import time
    s = time.time()
